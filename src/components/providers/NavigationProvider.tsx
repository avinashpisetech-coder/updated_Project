...
// These must exactly match the sidebar widths declared in Sidebar.tsx 
// so that content areas are always true-full-screen.
...
function applySidebarTokens(open: boolean, mode: "horizontal" | "vertical") {
  ...
  if (mode === "horizontal") {
    ...
    const expandedPx = SIDEBAR_EXPANDED_PX;
    const collapsedPx = SIDEBAR_COLLAPSED_PX_TICKET;
...
export function NavigationProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    applySidebarTokens(isSidebarOpen, navMode);
  }, [isSidebarOpen, navMode]);
