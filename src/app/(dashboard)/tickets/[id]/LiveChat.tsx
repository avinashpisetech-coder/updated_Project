    ...
    <div className="flex flex-col h-[450px]">
      <div className="flex-1 space-y-6 overflow-y-auto p-4 no-scrollbar">
        <div className="my-6 flex items-center justify-center">
          <span className="px-4 py-1.5 rounded-full bg-slate-100 text-[10px] font-bold uppercase tracking-widest text-slate-500 border border-slate-200">Session Initialized</span>
        </div>
        {messages.map((msg) => {
          const isMe = msg.sender_id === currentUserId;
          return (
            <div key={msg.id as string} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
              <span className="mx-2 mb-1.5 text-[10px] font-bold uppercase tracking-tight text-slate-400">
                {isMe ? 'Internal' : (msg.sender as { full_name: string })?.full_name || 'System User'} • {format(new Date(msg.created_at as string), 'HH:mm')}
              </span>
              <div 
                className={`p-4 rounded-xl max-w-[85%] text-[13px] font-medium leading-relaxed shadow-sm transition-all ${
                  isMe 
      ...
      <div className="mt-4 pt-6 border-t border-slate-200">
        <form onSubmit={sendMessage} className="relative flex items-center gap-3">
          <Input
            type="text"
            placeholder="Enter message for record..."
            value={newMessage}
            onChange={(e) => setNewMessage(e.target.value)}
            className="h-12 w-full rounded-xl border-slate-200 bg-white px-4 text-[13px] font-medium placeholder:text-slate-400 focus-visible:ring-indigo-500/20"
          />
          <Button
            type="submit"
            disabled={!newMessage.trim()}
            className="h-12 w-12 rounded-xl p-0 bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-100 transition-all flex items-center justify-center shrink-0"
          >
