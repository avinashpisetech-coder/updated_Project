...
export default function OrganizationManager({
  ...
  const handleCreateProject = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, name, code, status, company_id, company:companies!company_id(name)")
      .single();
    setSaving(false);
            ...
            <div className="flex flex-col md:flex-row items-end gap-4">
              <div className="flex-1 space-y-2 w-full">
                ...
                <Input 
                  ...
                  className="h-12 px-4 rounded-xl bg-background border-border/40 focus:border-primary/30 text-[13px] font-medium tracking-tight"
                />
              </div>
              <div className="w-full md:w-64 space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-primary/70 ml-1">Code</Label>
                <Input 
                  value={companyCode} 
              ...
              <Button 
                ...
                className="h-12 w-full md:w-auto px-10 rounded-xl bg-primary text-white font-bold uppercase tracking-wider text-[11px] hover:-translate-y-0.5 shadow-lg shadow-primary/20 transition-all active:scale-95 whitespace-nowrap"
              >
            ...
            <Table>
              <TableHeader className="bg-muted/30">
                <TableRow className="border-border/40 hover:bg-transparent">
                  <TableHead className="text-[10px] font-bold uppercase tracking-widest h-12 text-muted-foreground">Company Name</TableHead>
                  <TableHead className="text-[10px] font-bold uppercase tracking-widest h-12 text-muted-foreground">Code</TableHead>
              ...
              <TableBody>
                {companies.map((company) => (
                  <TableRow key={company.id} className="border-border/40 group hover:bg-primary/5 transition-colors">
                    ...
                    <TableCell className="font-mono text-[11px] font-bold text-muted-foreground">{company.code || "NON_REF"}</TableCell>
                    <TableCell>
        ...
        <TabsContent value="projects" className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row items-end gap-4">
              <div className="w-full md:w-64 space-y-2">
                ...
                <Select value={selectedCompanyId} onValueChange={setSelectedCompanyId}>
                  <SelectTrigger className="h-12 px-4 rounded-xl bg-background border-border/40 focus:border-primary/30 text-[13px] font-medium tracking-tight">
          ...
          <div className="rounded-xl border border-border/40 bg-card/60 overflow-hidden shadow-sm">
            <Table>
              <TableHeader className="bg-muted/30">
                <TableRow className="border-border/40 hover:bg-transparent">
                  <TableHead className="text-[10px] font-bold uppercase tracking-widest h-12 text-muted-foreground">Project Name</TableHead>
                  <TableHead className="text-[10px] font-bold uppercase tracking-widest h-12 text-muted-foreground">Code</TableHead>
              ...
              <TableBody>
                {projectRows.map((project) => (
                  <TableRow key={project.id} className="border-border/40 group hover:bg-primary/5 transition-colors">
                    ...
                    <TableCell className="font-mono text-[11px] font-bold text-muted-foreground">{project.code || "NON_REF"}</TableCell>
                    <TableCell>
