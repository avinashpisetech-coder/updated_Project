-- v123_nuclear_purge_protocol.sql
-- Description: Aggressively PURGE all name-clashing functions and redeploy definitive protocol.
...
BEGIN
    ...
    LOOP
        RAISE NOTICE 'Dropping function: %', r.func_spec;
...
BEGIN
    ...
    v_item_ids := ARRAY(
        ...
        WHERE (x->>'id') IS NOT NULL AND (x->>'id') ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{12}$'
    );

    -- 3. Delete safe materials
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        IF (v_item->>'id') IS NOT NULL AND (v_item->>'id') ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
