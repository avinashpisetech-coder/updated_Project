-- v223_optimize_analytics_rpc.sql
-- Description: Refactor get_advanced_analytics for better performance by optimizing subqueries and joins.
...
BEGIN
    ...
    RETURN (
        WITH filtered_tickets AS (
            ...
            -- Optimization: Join with assignee profile once instead of per-row subquery
            LEFT JOIN public.profiles ap ON ap.id = t.assigned_to_id
            WHERE (
                v_is_admin 
                OR (v_role = 'dept_admin' AND (
                    rp.department_id = v_dept_id
                    OR ap.department_id = v_dept_id
                    -- Note: Historical context might still need creator check if relevant
                ))
        ...
        task_stats AS (
            -- Optimization: Avoid EXISTS subquery if possible
            SELECT 
                count(*) as total,
            ...
            WHERE v_is_admin
               ...
               -- If not admin/creator, check if assigned
               OR tk.id IN (SELECT task_id FROM public.task_assignees WHERE profile_id = p_profile_id)
        ),
        user_stats AS (
            SELECT 
                count(*) as total,
        ...
        SELECT jsonb_build_object(
            ...
            'live_users', (SELECT jsonb_agg(jsonb_build_object('id', id, 'full_name', full_name, 'avatar_url', avatar_url, 'role', role)) FROM (SELECT id, full_name, avatar_url, role FROM public.profiles WHERE status = 'active' LIMIT 5) p)
        )
    );
END;
$$;
