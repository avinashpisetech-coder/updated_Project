-- v071_analytical_expansion.sql
-- Description: Expansion of analytical schema to support CSAT, Response/Resolution telemetry, and Channel distributions.

-- 1. Schema Expansion
ALTER TABLE public.tickets 
ADD COLUMN IF NOT EXISTS satisfaction_rating INTEGER CHECK (satisfaction_rating >= 1 AND satisfaction_rating <= 5),
ADD COLUMN IF NOT EXISTS origin TEXT NOT NULL DEFAULT 'Portal' CHECK (origin IN ('Portal', 'Email', 'Chat', 'API', 'Manual')),
ADD COLUMN IF NOT EXISTS first_response_at TIMESTAMPTZ;

-- 2. Backfill with Randomized Telemetry (For UI verification)
UPDATE public.tickets 
SET 
  origin = (ARRAY['Email', 'Portal', 'Chat', 'API', 'Manual'])[floor(random() * 5 + 1)],
  satisfaction_rating = CASE WHEN status IN ('resolved', 'closed') THEN floor(random() * 3 + 3) ELSE NULL END,
  first_response_at = created_at + (random() * interval '4 hours')
WHERE created_at > now() - interval '30 days';

-- 3. Redefine get_advanced_analytics RPC
