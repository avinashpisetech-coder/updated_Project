-- v128_allow_po_item_quantity_update.sql
-- Description: Update update_purchase_order_v2 to allow quantity synchronization for existing line items.
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
        IF (v_item->>'id') IS NOT NULL AND (v_item->>'id') ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
            ...
            SET quantity = (v_item->>'quantity')::numeric, -- Added support for quantity updates
