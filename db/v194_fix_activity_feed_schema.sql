-- v194_fix_activity_feed_schema.sql
-- Description: Fix "ta.actor_id" error in unified activity feed RPC.
...
BEGIN
  ...
  WITH combined_activities AS (
    ...
    (
      ...
      -- Tighten visibility: Only show if Admin, Creator, or Assignee
      WHERE (
        v_is_admin
        OR tk.created_by = v_user_id
        OR EXISTS (
          SELECT 1 FROM public.task_assignees tan 
          WHERE tan.task_id = tk.id AND tan.profile_id = v_user_id
        )
      )
