-- v121_referential_po_sync_protocol.sql
-- Description: Transition from 'Delete-All' to 'Referentially Aware Sync' for PO Amendments.
...
BEGIN
    ...
    SET po_number = p_po_number,
        ...
        po_type = p_po_type,
        branch = p_branch,
        reference = p_reference, -- New Reference field
        notes = p_notes,
        status = p_status,
    ...
    -- 2. Build array of incoming IDs to keep
    v_item_ids := ARRAY(
        SELECT (x->>'id')::uuid 
    ...
    -- 5. Trigger standard totals recalculation (handled by DB triggers)
END;
$$ LANGUAGE plpgsql;
